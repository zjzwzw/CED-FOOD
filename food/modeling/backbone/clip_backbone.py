...
class AttentionPool2d(nn.Module):
    ...
    #
    #         b, c, h, w = x.shape
    #         x_local = x.reshape(b,c,h*w).permute(0,2,1)
    #         # x_local = cp.checkpoint(self.v_proj,x_local)
    #         # x_local = cp.checkpoint(self.c_proj,x_local)
    #         # x_local = self.v_proj(x_local)
    ...
    #         return x[0], x_local
    #
    #     x, x_local = cp.checkpoint(_inner_forward, x)
    #     return x, x_local
    def forward(self, x):
...
class CLIPLangEncoder(nn.Module):
    ...
    def encode_prompt(self, prompts, tokenized_prompts):
