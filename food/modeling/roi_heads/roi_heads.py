...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        # self.ID = nn.Parameter(0.01*torch.randn(2048,128),requires_grad = True)
        # self.OOD = nn.Parameter(0.01*torch.randn(2048,128), requires_grad=True)
        # self.BG = nn.Parameter(0.01*torch.randn(2048,128), requires_grad=True)


        # self.attnpool = AttentionPool2d(4, 2048, 32, 1024)
        # self.layer = nn.Linear(2048,1024)
    ...
    def forward(self, images, features, proposals, targets, hooks):
        """
        ...
        # feature_pooled = self.attnpool(box_features)
        feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1 4096*2048 16384*2048
        # feature_pooled = self.layer(feature_pooled)
        ...
        #
        #     ID_mask = gt_classes != 21
        #     ID_samples = feature_pooled[ID_mask]
        #     ReC_ID_ID = ID_samples @ self.ID @ self.ID.T
        #     ReC_ID_OOD = ID_samples @ self.OOD @ self.OOD.T
        #
        #     BG_samples = feature_pooled[~ID_mask]
        #     objectness_logits = objectness_logits[~ID_mask]
        #     bg_objectness, bg_ind = (-objectness_logits).topk(30 if 30<objectness_logits.size(0) else objectness_logits.size(0))
        #     BG_samples = BG_samples[bg_ind]
        #     ReC_BG_BG = BG_samples @ self.BG @ self.BG.T
        #     ReC_BG_OOD = BG_samples @ self.OOD @ self.OOD.T
        #
        #     loss_zero = {"loss_zero":0.1*(torch.abs(torch.sum(torch.abs(self.ID.T @ self.ID))+
        #                                             torch.sum(torch.abs(self.BG.T @ self.BG))+
        #                                             torch.sum(torch.abs(self.OOD.T @ self.OOD))-0.01))}
        #
        #     ID_ID_MSE = F.mse_loss(ID_samples,ReC_ID_ID, reduction="mean")/torch.norm(ID_samples)
        #     BG_BG_MSE = F.mse_loss(BG_samples,ReC_BG_BG, reduction="mean")/torch.norm(BG_samples)
        #
        #
        #     reverse_ID = -torch.mean(ReC_ID_OOD)
        #     reverse_BG = -torch.mean(ReC_BG_OOD)
        #     ID_OOD_MSE = F.mse_loss(ID_samples,ReC_ID_OOD, reduction="mean")/torch.norm(ID_samples)+0.01*reverse_ID
        #     BG_OOD_MSE = F.mse_loss(BG_samples, ReC_BG_OOD, reduction="mean") / torch.norm(BG_samples)+0.01*reverse_BG
        #
        #     loss_mse = {"loss_mse": ID_ID_MSE+BG_BG_MSE+0.01*torch.norm(self.ID)**2+0.01*torch.norm(self.BG)**2}
        #     loss_ood = {"loss_ood": ID_OOD_MSE+BG_OOD_MSE+0.01*torch.norm(self.OOD)**2}
        ...
        #     pred_class_logits.max(dim=1).values.sum().backward(retain_graph=True)
        ...
        #     gradients2 = [grad.mean(dim=(-1,-2)) for grad in gradients]
        #     component = torch.abs(torch.cat(gradients2,dim=1))
        #     square_scores2 = torch.pow(component,2).mean(dim=1)
        #     scores = [grad.mean(dim=(-1,-2)) for grad in gradients1]
        #     scores = torch.cat(scores, dim=1)
        #     scores = torch.pow(scores, 2)
        #     square_scores1 = scores.mean(dim=(-1))
        #     square_scores = square_scores2*square_scores1
        #
        #
        #     # +energy
        #     # energy = -torch.logsumexp(pred_class_logits, dim=1)
        #     # square_scores = (1-square_scores)*energy
        #
        #     gt_classes = torch.cat([x.gt_classes for x in proposals]).cpu()
        #     mask = torch.isin(gt_classes, torch.tensor(list(range(10,15))))
        #
        #     # storage = get_event_storage()
        #     # if storage.iter < self.max_iter / 2:
        ...
        #     fg_score = pred_class_logits[fg_inds]
        #     # fg_obj = objectness[fg_inds]
        #     bg_square = square_scores[~fg_inds]
        #     bg_score = pred_class_logits[~fg_inds]
        #     # bg_obj = objectness[~fg_inds]
        #     _, pos_inds = fg_square.topk(3)
        #     _, neg_inds = bg_square.topk(3)
        #     # bg_square = bg_square[neg_inds]
        #     bg_score = bg_score[neg_inds]
        #     # bg_obj = bg_obj[neg_inds]
        #     # fg_square = fg_square[pos_inds]
        #     fg_score = fg_score[pos_inds]
        #     # fg_obj = fg_obj[pos_inds]
        #     # print(bg_obj,fg_obj)
        #     #
        #     # loss_grad_all = -torch.log(bg_scores)-torch.log(fg_scores)
        #     #
        #     # loss_grad = {"loss_grad": loss_grad_all.sum()/6}
        #
        #     # objectness[mask] = 0
        #     # square_scores = square_scores*objectness
        #
        #
        #     KLscores = torch.cat([fg_score,bg_score])
        #     logprob = F.log_softmax(KLscores, dim=1)
        #     loss_KL = -torch.sum(logprob,dim=1)/(self.num_classes+1)
        #     # print(loss_KL)
        #     loss_KL = {"loss_KL":0.1*loss_KL.sum()/KLscores.shape[0]}
        #     # print(loss_KL)
        #
        #     square_scores = torch.tensor(square_scores)
        square_scores = []
        ###
        ...
        if self.training:
            ...
            # unknown
            losses = self.box_predictor.losses(predictions, proposals, square_scores)
            # losses.update(loss_grad)
            # losses.update(loss_zero)
