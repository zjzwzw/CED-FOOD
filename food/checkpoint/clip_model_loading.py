...
def convert_basic_clip_names(original_keys, add_backbone_prefix=False, use_whole_clip=False, use_fpn_arch=False,
                             regionclip=False):
    """
...
def convert_clip_names(weights, add_backbone_prefix=False, use_whole_clip=False, use_fpn_arch=False, regionclip=False):
    ...
    layer_keys, use_vit = convert_basic_clip_names(layer_keys, add_backbone_prefix, use_whole_clip, use_fpn_arch,
                                                   regionclip)
...
def align_and_update_state_dicts_for_CLIP(model_state_dict, ckpt_state_dict, c2_conversion=True, bb_rpn_weights=False,
                                          regionclip=False):
    """
    ...
    if bb_rpn_weights:  # a 2nd pretrained weights to load, for offline backbone & RPN, then convert the ckpt key names and only keep the ones we need
        ...
        for original_k in ckpt_state_dict:
            ...
            if 'proposal_generator' in original_k:
                ...
                new_ckpt_state_dict[new_key] = ckpt_state_dict[original_k]
        new_ckpt_state_dict['ignore_others'] = torch.tensor(
            [1])  # ignore other model weights (not 'offline_*') in batch_norm.py
    ...
    ckpt_state_dict, original_keys, use_vit = convert_clip_names(ckpt_state_dict, add_backbone_prefix, use_whole_clip,
                                                                 use_fpn_arch, regionclip)
